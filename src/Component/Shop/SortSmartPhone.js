...
export default function SortSmartPhone() {
  ...
  useEffect(() => {
    axios.get('http://localhost:3010/pkdd/products')
      .then(response => {
        ...
        setFilteredProducts(sortedProducts.filter(product => product.category === "SmartPhone"));
      })
      .catch(error => {
  ...
  const handleLogoClick = (model) => {
    ...
    if (model) {
        const filtered = products.filter(product => product.model === model && product.category === "SmartPhone");
        setFilteredProducts(filtered);
        setFilters({ ...filters, name: '', model });
    } else {
        const filtered = products.filter(product => product.category === "SmartPhone");
        setFilteredProducts(filtered);
        ...
        <div className="container">
          <div className="row">
            <h4 className="text-center mb-4">
              <span className="px-2 position-relative fw-bolder fs-1">SmartPhone</span>
            </h4>
